...
class IGSO3():
    ...
    def __init__(self, *, T, min_sigma, max_sigma, min_b, max_b,
        ...
        # Pre-convert lookup tables to torch for GPU-resident score computation
        self._discrete_omega = torch.from_numpy(
            self.igso3_vals['discrete_omega']).float()
        self._discrete_sigma = torch.from_numpy(
            self.igso3_vals['discrete_sigma']).float()
