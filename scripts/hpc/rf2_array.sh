...
if [ "$#" -lt 3 ]; then
  echo "Usage: $0 <input.qv> <output_dir> <num_shards> [rf2 args...]" >&2
  exit 1
fi
...
OUTPUT_SHARD="$OUTPUT_DIR/rf2_shard_${TASK_ID}.qv"
...
mkdir -p "$OUTPUT_DIR"
echo "Running RF2 shard $TASK_ID: $INPUT_SHARD -> $OUTPUT_SHARD"
uv run rf2 \
